...
class Expression():
	...
	def solveExpression(self, group, expression):
		...
		if (resultGroup[origin].getState() == TRUE):
			...
			return False,False,resultGroup
		...
		if condition.count("=") > 0:
			...
			if (resultGroup[target].getState() == NOT_SURE):
			...
			elif (resultGroup[target].getState() != resultGroup[value].getState()) and resultGroup[value].getState() == NOT_SURE:
				...
				resultGroup[origin].setState(resultGroup[origin].getOppositeState())
				resultGroup[target].setState(NOT_SURE)
				resultGroup[value].setState(NOT_SURE)
...
def cloneGroup(group):
	...
	#print "Clone: "+str(newGroup)+" origin: "+str(group)
	return newGroup

def getResultGroups(groupList, group, index):
	for state in [NOT_SURE, TRUE, FALSE]:
		...
		if index == len(group)-1:
			...
			getResultGroups(groupList, group, index+1)
...
for case in range(1,int(raw_input())+1):          # For all test cases
	...
	getResultGroups(resultGroups, group, 0)
	print resultGroups
